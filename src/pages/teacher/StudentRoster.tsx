...
const StudentRoster = () => {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        (data || []).forEach((d) => {
          counts[d.student_id] = (counts[d.student_id] || 0) + 1;
        });
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        Object.entries(map).forEach(([k, v]) => {
          result[k] = { attempts: v.total, avgScore: Math.round(v.sum / v.total) };
        });
        setQuizData(result);
      });
    supabase
  ...
  const getStudentMetrics = (sId: string, sName: string): StudentMetrics => {
    ...
    const avgMastery =
      scores.length > 0 ? Math.round(scores.reduce((s, m) => s + m.masteryScore, 0) / scores.length) : 0;
    const sCheckins = checkins.filter((c) => c.studentId === sId);
    const avgHappiness =
      sCheckins.length > 0
        ? Number((sCheckins.reduce((s, c) => s + c.happinessScore, 0) / sCheckins.length).toFixed(1))
        : 0;
    return {
      id: sId,
      name: sName,
  ...
  const allStudents = [
    ...
    ...allDbStudents
      ...
      .map((ps) => ({
        ...
        classLevel: ps.preferred_class_level,
        section: ps.preferred_section,
  ...
  return (
    <div className="animate-fade-in space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {allDbStudents.filter((ps) => !ps.is_verified && !students.some((s: any) => s.id === ps.id)).length > 0 && (
              <Badge variant="outline" className="text-warning border-warning">
                {allDbStudents.filter((ps) => !ps.is_verified && !students.some((s: any) => s.id === ps.id)).length}{" "}
                pending
              </Badge>
            )}
                  ...
                  <TableRow key={s.id} className={isPending ? "bg-warning/5" : ""}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <div
                          className={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-bold ${isPending ? "bg-warning/10 text-warning" : "bg-primary/10 text-primary"}`}
                        >
                          {s.name.charAt(0)}
                        </div>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-xs font-medium ${m.avgMastery < 50 ? "text-destructive" : "text-success"}`}
                        >
                          {m.avgMastery}%
                        </span>
                      </div>
                    ...
                    <TableCell className="text-sm">
                      {m.badgeCount > 0 ? <Badge variant="secondary">{m.badgeCount}</Badge> : "—"}
                    </TableCell>
                    <TableCell>
                      <div className="flex gap-1">
                        <Button variant="ghost" size="sm" onClick={() => setShowView(m)} title="View details">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            openEdit({
                              ...
                              preferred_class_level: s.classLevel,
                              preferred_section: s.section,
                            })
                          }
                          title="Edit"
                        >
                        ...
                        {isPending && (
                          <Button variant="ghost" size="sm" onClick={() => handleVerifyStudent(s.id)} title="Verify">
                            <ShieldCheck className="h-3 w-3 text-success" />
                          </Button>
                        )}

                        <Button variant="ghost" size="sm" onClick={() => handleRemoveStudent(s.id)} title="Remove">
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              Student will be auto-verified and added to the selected class.
            </p>
            <Button onClick={handleCreateStudent} className="w-full" disabled={creating}>
              {creating ? "Creating…" : "Create New Student"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!showEdit} onOpenChange={() => setShowEdit(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Edit Student — {showEdit?.name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>Full Name</Label>
      ...
      <Dialog open={!!showView} onOpenChange={() => setShowView(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Student Details — {showView?.name}</DialogTitle>
          </DialogHeader>
          {showView && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <MetricCard
                  ...
                  value={`${showView.avgMastery}%`}
                  label="Avg Mastery"
                />
                <MetricCard
                  icon={<SmilePlus className="h-5 w-5 text-warning" />}
                  value={showView.avgHappiness > 0 ? `${showView.avgHappiness}` : "—"}
                  label="Happiness"
                />
                <MetricCard
                  icon={<BookOpen className="h-5 w-5 text-accent" />}
                  value={`${showView.quizAttempts}`}
                  label="Quizzes Taken"
                />
                <MetricCard
                  icon={<BarChart3 className="h-5 w-5 text-success" />}
                  value={showView.quizAvgScore > 0 ? `${showView.quizAvgScore}%` : "—"}
                  label="Quiz Avg Score"
                />
                <MetricCard
                  icon={<Trophy className="h-5 w-5 text-accent" />}
                  value={`${showView.challengesSubmitted}`}
                  label="Challenges Done"
                />
                <MetricCard
                  icon={<Trophy className="h-5 w-5 text-success" />}
                  value={`${showView.badgeCount}`}
                  label="Badges Earned"
                />
                <MetricCard
                  icon={<CheckCircle className="h-5 w-5 text-primary" />}
                  value={`${showView.lessonsVerified}`}
                  label="Lessons Verified"
                />
              </div>
            </div>
          )}
